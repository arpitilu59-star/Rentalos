    ...
    <AppShell>
      <div className="space-y-6">
        <PageHeader
          ...
          subtitle="Track deposits held and refund settlements."
        />

        <form
          onSubmit={submit}
          className="rounded-2xl bg-card border border-border p-5 shadow-card space-y-3"
        >
                ...
                >
                  <div>
                    ...
                    <div className="text-xs mt-1">
                      ...
                      {Number(d.amount_deducted) > 0 && (
                        <> · Deducted: {formatINR(Number(d.amount_deducted))}</>
                      )}
                      {Number(d.amount_refunded) > 0 && (
                        <> · Refunded: {formatINR(Number(d.amount_refunded))}</>
                      )}
                    </div>
                    {d.deduction_reason && (
                    ...
                    <span
                      className={`inline-block mt-1 text-[10px] px-1.5 py-0.5 rounded ${d.status === "refunded" ? "bg-emerald-500/10 text-emerald-600" : d.status === "forfeited" ? "bg-destructive/10 text-destructive" : "bg-amber-500/10 text-amber-600"}`}
                    >
                      {d.status.replace("_", " ")}
                    </span>
                  </div>
