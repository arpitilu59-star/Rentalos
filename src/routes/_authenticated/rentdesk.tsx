...
import { DoorOpen, Users, Wallet, CheckCircle2, Clock, Search, Receipt } from "lucide-react";
...
function Dashboard() {
  ...
  const [bills, setBills] = useState<
    (Bill & { rooms: { room_number: string } | null; tenants: { name: string } | null })[]
  >([]);
  ...
  useEffect(() => {
    (async () => {
      ...
      const pendingAmount = b
        .filter((x) => x.status !== "paid")
        .reduce((s, x) => s + (Number(x.total_amount) - Number(x.amount_paid || 0)), 0);
      const paidAmount = b
        .filter((x) => x.status === "paid")
  ...
  const filteredTenants = q
    ? tenants.filter(
        (x) =>
          ...
          x.phone.includes(q) ||
          (x.rooms?.room_number ?? "").toLowerCase().includes(q),
      )
    : tenants;
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <StatCard
            ...
            value={formatINR(stats?.monthIncome ?? 0)}
            icon={Wallet}
            accent="text-success"
          />
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div className="rounded-2xl bg-card border border-border p-5 shadow-card">
            ...
            {bills.length === 0 ? (
              <EmptyState icon={Receipt} title="Abhi koi bills nahi." />
            ) : (
                  ...
                  <li key={b.id} className="py-2.5 flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      <div className="font-medium truncate">
                        {b.tenants?.name || "Tenant"} · Room {b.rooms?.room_number}
                      </div>
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <div
                        className={`text-[10px] uppercase tracking-wide ${b.status === "paid" ? "text-success" : "text-warning"}`}
                      >
                        {b.status}
                      </div>
                    </div>
            ...
            <div className="flex items-center justify-between gap-3 mb-3">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Search…"
                  className="pl-7 pr-2 py-1.5 text-xs rounded-md bg-background border border-input outline-none focus:ring-2 ring-ring/40 w-36"
                />
              </div>
            </div>
            {filteredTenants.length === 0 ? (
              <EmptyState icon={Users} title="Koi tenant nahi mila." />
            ) : (
              <ul className="divide-y divide-border text-sm">
                  ...
                  <li key={t.id} className="py-2.5 flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-muted-foreground">
                        Room {t.rooms?.room_number} · {t.phone}
                      </div>
                    </div>
