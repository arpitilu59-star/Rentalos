...
} from "@/components/BillHistoryFilters";

export const Route = createFileRoute("/_authenticated/bills")({ component: BillsPage });

type BillRow = Bill & {
  rooms: { room_number: string } | null;
  tenants: { name: string; phone: string; email: string | null; move_in_date: string } | null;
};
...
function BillsPage() {
  ...
  const [ymFilter, setYmFilter] = useState<BillFilterState>({
    year: "all",
    month: "all",
    status: "all",
  });
  ...
  const load = async () => {
    const [{ data: b }, { data: r }, { data: t }, { data: m }, { data: p }, s] = await Promise.all([
      supabase
        .from("bills")
        .select("*, rooms(room_number), tenants(name, phone, email, move_in_date)")
  ...
  const markPaid = async (b: BillRow) => {
    ...
    await supabase
      .from("bills")
      .update({ status: "paid", amount_paid: total, paid_at: new Date().toISOString() })
      .eq("id", b.id);
    const r = await sendReceipt(b, total);
    if (r.sent) alert("Marked paid. Receipt emailed to tenant ✓");
    else if (r.reason === "no-email")
      alert("Marked paid. (Tenant has no email — add it to send receipt automatically.)");
  ...
  const pauseReminders = async (b: BillRow) => {
    const dStr = prompt(
      "Stop reminders until (YYYY-MM-DD)?",
      toISODate(new Date(Date.now() + 14 * 86400000)),
    );
    if (!dStr) return;
  ...
  const statusFiltered = bills.filter((b) =>
    filter === "all"
      ? true
      : filter === "dues"
        ? b.status !== "paid" && Number(b.previous_dues) > 0
        : b.status === filter,
  );
  const filtered = useBillFilter(statusFiltered, ymFilter);
  const totalDues = bills
  ...
  return (
    <AppShell>
      <PageHeader
        title="Bills"
        subtitle="Generate bills, accept payments & track dues."
        action={
          <Button onClick={() => setShowForm(true)} disabled={rooms.length === 0 || !settings}>
      ...
      {totalDues > 0 && (
        <div className="mb-4 rounded-2xl bg-destructive/10 border border-destructive/30 p-4 flex items-center justify-between">
          <div>
            <div className="text-xs uppercase tracking-wide text-destructive font-medium">
              Total Outstanding
            </div>
      ...
      <div className="space-y-3">
        {filtered.map((b) => (
          <BillCard
            ...
            onMarkPending={() => markPending(b)}
            onRecordPayment={() => setPayFor(b)}
            onPauseReminders={() => pauseReminders(b)}
            onDelete={() => remove(b.id)}
            onChanged={load}
          />
        ))}
        {filtered.length === 0 && <EmptyState title="No bills here." />}
      </div>
...
function BillCard({
  ...
  const upi = upiPayUrl(
    ...
    balance,
    `Rent Room ${bill.rooms?.room_number}`,
  );
  const qr = upiQrUrl(
    profile?.upi_id,
    profile?.business_name || profile?.full_name,
  ...
  const sendTwilio = async () => {
    if (!profile?.whatsapp_from) {
      alert("Profile → 'WhatsApp sender number' set karo (Twilio sandbox: +14155238886).");
      return;
    }
    if (!phone) {
      alert("Tenant phone missing.");
      return;
    }
    setSending(true);
    try {
      ...
      await supabase
        ...
        .eq("id", bill.id);
      alert(
        mediaUrl ? "WhatsApp message + PDF sent ✓" : "WhatsApp message sent (PDF attach failed) ✓",
      );
      onChanged();
    } catch (e) {
  ...
  return (
    <div className="rounded-2xl bg-card border border-border p-4 shadow-card">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="text-right">
          ...
          <div className="text-[10px] text-muted-foreground">
            of {formatINR(Number(bill.total_amount))}
          </div>
          <span
            className={`inline-block mt-0.5 text-[10px] px-2 py-0.5 rounded-full ${statusBadge}`}
          >
            {bill.status}
          </span>
        </div>
      ...
      <div className="mt-3 grid sm:grid-cols-3 gap-2 text-xs">
        ...
        <Line
          label={`Elec (${bill.units_consumed}u)`}
          value={formatINR(Number(bill.electricity_amount))}
        />
        ...
        {Number(bill.other_charges) > 0 && (
          <Line
            label={bill.other_charges_note || "Other"}
            value={formatINR(Number(bill.other_charges))}
          />
        )}
        {Number(bill.previous_dues) > 0 && (
          <Line label="Previous dues" value={formatINR(Number(bill.previous_dues))} />
        )}
        {Number(bill.amount_paid) > 0 && (
          <Line label="Paid" value={formatINR(Number(bill.amount_paid))} />
        )}
      </div>

      {bill.reminders_paused_until && (
        <div className="mt-2 text-[11px] text-muted-foreground inline-flex items-center gap-1">
          <BellOff className="size-3" /> Reminders paused till{" "}
          {formatDate(bill.reminders_paused_until)}
        </div>
      )}
      {bill.receipt_sent_at && (
        <div className="mt-1 text-[11px] text-success inline-flex items-center gap-1">
          <Mail className="size-3" /> Receipt emailed {formatDate(bill.receipt_sent_at)}
        </div>
      )}

      <div className="mt-4 flex flex-wrap gap-2">
        <a
          href={phone ? waUrl : undefined}
          target="_blank"
          rel="noreferrer"
        ...
        <button
          onClick={sendTwilio}
          disabled={sending}
          className="inline-flex items-center gap-1.5 rounded-lg bg-primary text-primary-foreground px-3 py-1.5 text-xs font-medium disabled:opacity-60"
        >
          <Send className="size-3" /> {sending ? "Sending…" : "Auto-send"}
        </button>
        <button
          onClick={() => {
            void downloadBillPdf(bill, profile);
          }}
        ...
        <button
          onClick={bill.status === "paid" ? onMarkPending : onMarkPaid}
          className="inline-flex items-center gap-1.5 rounded-lg border border-border px-3 py-1.5 text-xs hover:bg-accent"
        >
          <CheckCircle2 className="size-3" /> Mark{" "}
          {bill.status === "paid" ? "pending" : "fully paid"}
        </button>
        {bill.status !== "paid" && (
          <button
            onClick={onPauseReminders}
            className="inline-flex items-center gap-1.5 rounded-lg border border-border px-2 py-1.5 text-xs hover:bg-accent"
        ...
        <button
          onClick={onDelete}
          className="inline-flex items-center gap-1.5 rounded-lg border border-border px-2 py-1.5 text-xs hover:bg-destructive/10 hover:text-destructive ml-auto"
        >
        ...
        <div className="mt-3 rounded-xl border border-border p-3 bg-muted/30 flex flex-col sm:flex-row gap-3 items-center">
          ...
          <div className="flex-1 text-xs space-y-1.5 w-full">
            <div className="text-muted-foreground">
              Pay <span className="font-semibold text-foreground">{formatINR(balance)}</span> to
            </div>
            <div className="flex items-center gap-2">
              <code className="flex-1 px-2 py-1.5 rounded bg-background border border-border">
                {profile.upi_id}
              </code>
              <button
                onClick={() => {
                  navigator.clipboard.writeText(profile.upi_id!);
                  alert("UPI ID copied");
                }}
                className="rounded border border-border px-2 py-1.5 hover:bg-accent"
              >
                <Copy className="size-3" />
              </button>
            </div>
                ...
                onClick={() => {
                  ...
                  setTimeout(() => {
                    ...
                    if (document.visibilityState === "visible") {
                      ...
                      alert(
                        "Koi UPI app installed nahi mili. Link copy ho gaya — phone pe paste karo, ya QR scan karo.",
                      );
                    }
                  }, 1200);
                }}
            ...
            <p className="text-[10px] text-muted-foreground">
              Mobile pe UPI app khulega. Desktop pe QR scan karo.
            </p>
          </div>
        </div>
      )}
...
function Line({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between rounded-lg bg-muted/40 px-2.5 py-1.5">
...
function RecordPaymentForm({
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    await supabase
      .from("payments")
      .insert({ owner_id, bill_id: bill.id, amount: amt, method, note: note || null });
...
function GenerateBillForm({
  ...
  const prev = roomReadings[1];

  // Anniversary-based defaults
  const range = tenant?.move_in_date
    ? anniversaryRange(tenant.move_in_date)
    : (() => {
        const n = nextMonthRange();
        const p = prevMonthRange();
        return { rentStart: n.start, rentEnd: n.end, elecStart: p.start, elecEnd: p.end };
      })();
  ...
  const [prev_reading, setPrev] = useState(
    prev?.reading?.toString() ?? tenant?.initial_reading?.toString() ?? "0",
  );
  ...
  const [persons, setPersons] = useState(
    roomTenants.reduce((s, t) => s + t.persons, 0).toString() || "1",
  );
  const [due_date, setDue] = useState(
    toISODate(
      new Date(
        range.rentStart.getFullYear(),
        range.rentStart.getMonth(),
        range.rentStart.getDate() + 5,
      ),
    ),
  );
  ...
  useEffect(() => {
    (async () => {
      ...
      const sum = (data ?? []).reduce(
        (s: number, b: { total_amount: number; amount_paid: number }) =>
          s + (Number(b.total_amount) - Number(b.amount_paid)),
        0,
      );
  ...
  useEffect(() => {
    ...
    if (t?.move_in_date) {
      ...
      setRentStart(toISODate(r.rentStart));
      setRentEnd(toISODate(r.rentEnd));
      setElecStart(toISODate(r.elecStart));
      setElecEnd(toISODate(r.elecEnd));
      setDue(
        toISODate(
          new Date(r.rentStart.getFullYear(), r.rentStart.getMonth(), r.rentStart.getDate() + 5),
        ),
      );
    }
  ...
  const water_per_person_rate =
    room?.water_per_person != null
      ? Number(room.water_per_person)
      : Number(settings.water_per_person);
  const water_amount = Number(persons) * water_per_person_rate;
  const cleaning_amount =
    room?.cleaning_amount != null ? Number(room.cleaning_amount) : Number(settings.cleaning_amount);
  const total_amount =
    Number(rent_amount) +
    electricity_amount +
    water_amount +
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    await supabase.from("bills").insert({
      ...
      rent_period_start: rentStart,
      rent_period_end: rentEnd,
      rent_amount: Number(rent_amount),
      elec_period_start: elecStart,
      elec_period_end: elecEnd,
      prev_reading: Number(prev_reading),
      curr_reading: Number(curr_reading),
      ...
      other_charges_note: other_charges_note || null,
        ...
        <form onSubmit={submit} className="p-5 space-y-3 max-h-[85vh] overflow-y-auto">
          ...
          {tenant?.move_in_date && (
            <div className="text-[11px] text-muted-foreground">
              Dates auto-set from move-in day ({new Date(tenant.move_in_date).getDate()}). Edit if
              needed.
            </div>
          )}
          ...
          <div className="rounded-xl bg-muted/40 p-3 space-y-2">
            ...
            <div className="text-xs text-muted-foreground">
              Units: <span className="font-semibold text-foreground">{units}</span> × ₹
              {settings.electricity_per_unit} ={" "}
              <span className="font-semibold text-foreground">{formatINR(electricity_amount)}</span>
            </div>
          </div>
          ...
          <Field label="Previous dues (unpaid balance)">
            ...
            <span className="text-[10px] text-muted-foreground mt-1 block">
              Auto-filled from unpaid balances.
            </span>
          </Field>

          <div className="rounded-xl bg-muted/40 p-3 space-y-2">
            <div className="text-xs font-medium text-muted-foreground">
              Other charges (parking, maintenance, damage, etc.)
            </div>
            <div className="grid grid-cols-[1fr_120px] gap-2">
              <input
                value={other_charges_note}
          ...
          <div className="rounded-xl border border-primary/30 bg-primary/5 p-3 text-sm space-y-1">
            ...
            {Number(other_charges) > 0 && (
              <Row k={other_charges_note || "Other"} v={formatINR(Number(other_charges))} />
            )}
            {Number(previous_dues) > 0 && (
              <Row k="Previous dues" v={formatINR(Number(previous_dues))} />
            )}
