...
function TenantsPage() {
  ...
  const filtered = tenants.filter(
    (t) =>
      !q ||
      (t.name + t.phone + (t.rooms?.room_number ?? "")).toLowerCase().includes(q.toLowerCase()),
  );
  ...
  return (
    <AppShell>
      <PageHeader
        title="Tenants"
        subtitle="Track who lives where."
        action={
          <Button
            onClick={() => {
              setEditing(null);
      ...
      {rooms.length === 0 && (
        <div className="mb-4">
          <EmptyState title="Add a room first." />
        </div>
      )}
      ...
      <div className="mb-3">
        <input
          ...
          placeholder="Search…"
          className="w-full px-3 py-2 rounded-lg bg-card border border-input outline-none focus:ring-2 ring-ring/40"
        />
      </div>
        ...
        {filtered.map((t) => (
          <div key={t.id} className="rounded-2xl bg-card border border-border p-4 shadow-card">
            <div className="flex items-start justify-between">
              ...
              <span
                className={`text-[10px] px-2 py-0.5 rounded-full ${t.active ? "bg-success/15 text-success" : "bg-muted text-muted-foreground"}`}
              >
            ...
            <div className="mt-2 text-xs text-muted-foreground space-y-0.5">
              <div>
                Persons: <span className="font-medium text-foreground">{t.persons}</span>
              </div>
...
function TenantForm({
  ...
  const [move_in_date, setMove] = useState(
    initial?.move_in_date ?? new Date().toISOString().slice(0, 10),
  );
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      move_in_date,
      rent_share: rent_share === "" ? null : Number(rent_share),
      active,
    ...
    if (!initial && initReadingNum != null) {
      await supabase.from("meter_readings").insert({
        owner_id,
        room_id,
        reading: initReadingNum,
        reading_date: move_in_date,
    ...
    <div className="fixed inset-0 z-50 bg-black/40 grid place-items-center p-4">
      <div className="bg-card border border-border rounded-2xl w-full max-w-md shadow-elevated">
        ...
        <form onSubmit={submit} className="p-5 space-y-3 max-h-[80vh] overflow-y-auto">
          ...
          <Field label="Rent share (optional)">
            <input
              type="number"
              min="0"
              value={rent_share}
          ...
          <div className="rounded-xl bg-muted/40 p-3 space-y-2 border border-dashed border-border">
            <div className="text-xs font-medium text-muted-foreground">
              Move-in meter reading (optional)
            </div>
            <p className="text-[11px] text-muted-foreground">
              Yahan meter ki current reading daalo jo move-in ke time thi. Future bills isse
              calculate honge (0 se nahi).
            </p>
            <input
              type="number"
              min="0"
            ...
            <button
              ...
              className="w-full inline-flex items-center justify-center gap-1.5 rounded-lg border border-border px-3 py-2 text-xs hover:bg-accent"
            >
