...
function PropertiesPage() {
  ...
  const del = async (id: string) => {
    if (!confirm("Property delete karein? Iske saare rooms, tenants, bills bhi delete ho jayenge."))
      return;
          ...
          <div key={p.id} className="rounded-2xl bg-card border border-border p-4 shadow-card">
            ...
            <div className="flex gap-2 mt-4 flex-wrap">
              ...
              <button
                ...
                title="Add / manage live verified video"
              >
              ...
              <button
                ...
                disabled={busy === p.id}
                className={`inline-flex items-center justify-center gap-1 rounded-lg px-2 py-1.5 text-xs font-medium disabled:opacity-60 ${p.is_public_listing ? "border border-border hover:bg-accent" : "bg-primary text-primary-foreground"}`}
              >
                {busy === p.id ? (
        ...
        >
          ...
          >
            <div className="flex items-center justify-between">
              <div className="font-semibold text-sm">{liveFor.name} — Live feed</div>
        ...
        <form onSubmit={submit} className="p-5 space-y-3">
          ...
          <div>
            ...
            {latitude != null && longitude != null && (
              ...
              >
                <MapPin className="size-3" /> View on map ({latitude.toFixed(4)},{" "}
                {longitude.toFixed(4)})
              </a>
            )}
          </div>
