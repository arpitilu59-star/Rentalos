      ...
      <aside className="hidden md:flex w-60 shrink-0 flex-col border-r border-border bg-card/40">
        ...
        <nav className="px-3 py-2 space-y-1 overflow-y-auto">
          ...
          {NAV.map(({ to, label, icon: Icon }) => (
            <Link
              key={to + label}
              to={to as never}
              className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm ${isActive(to) ? "bg-primary text-primary-foreground font-medium" : "hover:bg-accent text-foreground"}`}
            >
            ...
            <>
              ...
              {landlordExtras.map(({ to, label, icon: Icon }) => (
                <Link
                  key={to + label}
                  to={to as never}
                  className="flex items-center gap-3 rounded-lg px-3 py-2 text-sm hover:bg-accent text-foreground"
                >
                  <Icon className="size-4" /> {label}
                </Link>
              ))}
        ...
        <div className="mt-auto p-3 space-y-1 text-sm">
          {variant === "tenant" ? (
            <Link
              to="/myr/onboard"
              search={{ role: "landlord" } as never}
              className="flex items-center gap-2 rounded-lg px-3 py-2 hover:bg-accent text-primary"
            >
          ...
          <Link
            ...
            className="flex items-center gap-2 rounded-lg px-3 py-2 hover:bg-accent"
          >
            <UserCircle className="size-4" /> {name || "Profile"}
          </Link>
          <button
            onClick={logout}
            className="w-full flex items-center gap-2 rounded-lg px-3 py-2 text-destructive hover:bg-destructive/10"
          >
      ...
      <nav className="md:hidden fixed bottom-0 inset-x-0 z-40 bg-card border-t border-border flex justify-around py-1 overflow-x-auto">
        {NAV.map(({ to, label, icon: Icon }) => (
          <Link
            key={to + label}
            to={to as never}
            className={`flex flex-col items-center gap-0.5 px-3 py-1.5 text-[10px] rounded-md shrink-0 ${isActive(to) ? "text-primary" : "text-muted-foreground"}`}
          >
