...
export const submitLiveVideo = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { target, storage_path, captured_lat, captured_lng, random_prompt, duration_seconds, mime_type } = data;
    ...
    if (target.kind === "property") {
      const { data: p, error } = await supabase.from("properties")
        .select("id, latitude, longitude").eq("id", target.id).maybeSingle();
      ...
      const { data: r, error } = await supabase.from("rooms")
        .select("id, property_id, properties(latitude, longitude)").eq("id", target.id).maybeSingle();
      if (error || !r) throw new Error("Room not found or not yours");
      const prop = (r as unknown as { properties: { latitude: number | null; longitude: number | null } | null }).properties;
      target_lat = prop?.latitude ?? null;
      target_lng = prop?.longitude ?? null;
    } else if (target.kind === "myr_listing") {
      const { data: l, error } = await supabase.from("myr_listings")
        .select("id, landlord_id").eq("id", target.id).maybeSingle();
      if (error || !l) throw new Error("Listing not found or not yours");
    } else if (target.kind === "myr_room") {
      const { data: r, error } = await supabase.from("myr_listing_rooms")
        .select("id, listing_id").eq("id", target.id).maybeSingle();
      if (error || !r) throw new Error("Room not found or not yours");
    }
    ...
    if (target_lat != null && target_lng != null) {
      ...
      verification_status = distance_m <= MAX_DISTANCE_M ? "verified" : "flagged";
    }
    ...
    const { data: row, error: insErr } = await supabase
      .from("live_feed_videos").insert(insertRow).select("id, verification_status, distance_m").single();
    if (insErr) throw insErr;

    return { id: row.id as string, status: row.verification_status as string, distance_m: row.distance_m as number | null };
  });
...
export const listMyLiveVideos = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const col = { myr_listing: "myr_listing_id", myr_room: "myr_room_id", property: "property_id", room: "room_id" }[data.target.kind];
    const { data: rows, error } = await supabase.from("live_feed_videos")
      .select("id, storage_path, verification_status, distance_m, random_prompt, duration_seconds, created_at")
      .eq(col, data.target.id).order("created_at", { ascending: false });
...
export const deleteLiveVideo = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { supabase } = context;
    const { data: row } = await supabase.from("live_feed_videos").select("storage_path").eq("id", data.id).maybeSingle();
    if (row?.storage_path) {
...
export const getVerifiedVideoUrl = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: row } = await supabasePublic
      ...
      .select("storage_path, verification_status")
      .eq("id", data.id)
      .maybeSingle();
    if (!row || row.verification_status !== "verified") return { url: null as string | null };
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { data: sig } = await supabaseAdmin.storage.from("live-feed-videos").createSignedUrl(row.storage_path, 3600);
...
export const getCoverVideoId = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const col = { myr_listing: "myr_listing_id", myr_room: "myr_room_id", property: "property_id", room: "room_id" }[data.target.kind];
    const { data: row } = await supabasePublic.from("live_feed_videos")
      .select("id").eq(col, data.target.id).eq("verification_status", "verified")
